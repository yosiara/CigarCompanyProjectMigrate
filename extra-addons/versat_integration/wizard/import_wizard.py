...
class ImportWizard(TransientModel):
    ...
    def action_import_products(self):
        ...
        for row in cursor:
            ...
            if not account_dict.get(row[9], False) and account_dict.get(row[9], False) != None:
                values = {
                    'external_id': int(row[0]),
                    'code': row[1],
                    'name': ustr(row[2]),
                    'uom_id': int(versat_uom_dict.get(row[3])[1]),
                    'price': float(row[4]),
                    'price_extra': float(row[5]),
                    'category_id': int(category_dict.get(row[6], False)),
                    'account_id': int(account_dict.get(row[9], False)),
                }
            else:
                values = {
                    'external_id': int(row[0]),
