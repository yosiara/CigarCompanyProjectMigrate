...
class DBExternalConnectorTemplate(models.Model):
    ...
    port = fields.Char(string="Port456", required=True)
    ...
    connector = fields.Selection(
        CONNECTORS, "Connector", required=True, default="postgresql"
    )
    ...
    def conectar(self):
        ...
        if platform == "linux" and self.connector == "mssql":
            try:
                self._connection = Mssql.connect(
                    host=data["Server"],
                    user=data["uid"],
                    passwd=data["pwd"],
                    db=data["database"],
                ...
                raise UserError(
                    _(
                        "Connection failed!!. Check your data connection in Linux System 2."
                    )
                )

        if platform == "win32" and self.connector == "mssql":
            try:
                self._connection = Pypyodbc.connect(autocommit=True, **data)
                return self._connection
