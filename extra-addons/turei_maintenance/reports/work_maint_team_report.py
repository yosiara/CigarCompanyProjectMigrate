...
class WorkMaintTeamReport(ReportXlsx):
    @api.model
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        maint_team = self.env['maintenance.team'].search([])

        for mt in maint_team:
            work_order = self.env['turei_maintenance.work_order'].search([('opening_date', '>=', lines.date_start), ('opening_date', '<=', lines.date_end), ('maintenance_team_id', '=', mt.id)])
            worksheet = workbook.add_worksheet(tools.ustr(mt.name))
...
WorkMaintTeamReport('report.turei_maintenance.work_maint_team_report', 'wzd.work.maint.team')
