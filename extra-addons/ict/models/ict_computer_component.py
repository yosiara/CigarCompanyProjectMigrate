...
class ICTComputerComponent(models.Model):
    _name = 'ict.computer.component'
    _description = 'ICT Computer Component'
    ...
    # @api.depends('name', 'computer_id', 'inventory_number')
    # def get_qrimage(self):
    #     options = {'width': 100 * mm, 'height': 100 * mm}
    #     name = self.name if self.name else ''

    #     qr_code = _('Name: %s\n') % name
    #     qr_code += _('Assign To Equipment: %s\n') % self.computer_id.inventory_number
    #     qr_code += _('Local: %s\n') % (self.computer_id.local_id.name if self.computer_id.local_id else '')
