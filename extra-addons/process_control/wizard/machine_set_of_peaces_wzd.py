...
class WzdMachineSetOfPeacesToExcel(models.TransientModel):
    ...
    def export_to_xlsx(self):
        if self.group_by == 'seccion':
            return self.env['report'].get_action(self, 'process_control.machine_set_of_peaces_report', data={
                'start_date': self.start_date,
                'end_date': self.end_date,
                'group_by': self.group_by,
                'turn': self.turn.id,
            })
        return self.env['report'].get_action(self, 'process_control.machine_set_of_peaces_by_line_report', data={
                'start_date': self.start_date,
                'end_date': self.end_date,
                'group_by': self.group_by,
                'turn': self.turn.id,
