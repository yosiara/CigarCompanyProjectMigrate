...
class RejectionMod1(models.Model):
    ...
    def _onchange_productive_line_id(self):
        if self.productive_line_id.id is not self.machine_id.productive_line_id.id:
            rejection_mod1_recs = self.tech_control_id.rejection_mod1_ids
    ...
    def _onchange_tech_control_id(self):
        self.productive_line_id = self.productive_line_id.search([('productive_section_id', '=', self.tech_control_id.productive_section_id.id)], limit=1).id
    ...
    #         rejection_recs = self.tech_control_id.rejection_mod1_ids.sorted(key=lambda r: r.turn_attendance_id.hour_from, reverse=True)
