...
class ProductionByHours(models.Model):
    ...
    tech_control_id = fields.Many2one(comodel_name='process_control.tech_control', string='Tech Control *', ondelete='cascade', required=True)

    @api.onchange('tech_control_id')
    def _onchange_tech_control_id(self):
        domain = [('turn_id', '=', self.tech_control_id.turn_id.id), ('session', '=', self.tech_control_id.session)]
        production_by_hours_recs = self.tech_control_id.production_by_hours_ids.sorted(lambda r: r.turn_att_id.hour_from, reverse=True)
