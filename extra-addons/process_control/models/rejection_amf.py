...
class RejectionAMF(models.Model):
    _name = 'process_control.rejection_amf'
    ...
    def _onchange_tecnolog_control_id(self):
        ...
        if len(rejection_amf_recs) > 1:
            for line in range(len(line_recs)-1):
                if line_recs[line].id == rejection_amf_recs[-2].productive_line_id.id:
                    i = line + 1
                    break
    ...
    #         next_turn_attendance = turn_attendance_recs.filtered_domain([('hour_from', '>', rejection_recs[0].turn_attendance_id.hour_from)])
