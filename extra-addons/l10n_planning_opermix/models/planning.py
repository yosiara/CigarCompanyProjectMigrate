...
class Planning(models.Model):
    ...
    cumplimiento_usuario = fields.Boolean(string='Cumplido')
    cumplimiento_final = fields.Selection([('Cumplido', 'Cumplido'), ('Incumplido', 'Incumplido'), ('Derogado', 'Derogado')], string='Cumplimiento')
    controla_cumplimiento_id = fields.Many2one('hr.employee', string='Ctrl. Cumpl.')
    conformidad = fields.Boolean(string='Conformidad')
    fecha_revision = fields.Date(string='Fecha de Conformidad', default=fields.Date().today())
    mostrar_conformidad = fields.Boolean('Mostrar Conformidad', compute='_compute_controla_cumplimiento')
    info_cumplimiento = fields.Text(string='Inf. Cumplimiento')
    plan_trabajo = fields.Boolean(string='Plan Trabajo', default=False)
    nro_acuerdo = fields.Char(string='Nro. Acuerdo')
    planning_subcomision_id = fields.Many2one('planning.subcomisiones', string='Subcomisión', required=True)
    no_consecutivo = fields.Char(string='No.')
    fecha_cumplimiento = fields.Date(string='Fecha Cumplimiento', default=fields.Date().today())
    fecha_real = fields.Date(string='Cumpl. Real', default=fields.Date().today())
    mostrar_fecha_real = fields.Boolean('Mostrar Fecha Real')
    archivo_indicacion = fields.Binary(
        attachment=True,
        string="Archivo",
        copy=False,
    )
    adjunto_respuesta = fields.Binary(
        attachment=True,
        string="Adjuntar Rpta.",
        copy=False,
    )
    prorrogar = fields.Boolean(string='Prorrogar')
    fecha_solicitada = fields.Date(string='Fecha solicitud', default=fields.Date().today())
    causa_prorroga = fields.Text(string='Causa')
    nueva_fecha_propuesta = fields.Date(string='Fecha Propuesta')
    aceptar_prorroga = fields.Boolean(string='Aprobar')
    planning_prorrogation_ids = fields.One2many('planning.prorrogation', 'planning_slot_id', string='Prorrogas Aprobadas')

    def _compute_controla_cumplimiento(self):
        for record in self:
            record.mostrar_conformidad = False
            if record.controla_cumplimiento_id:
                if record.controla_cumplimiento_id.id == self.env.user.id or self.env.user.has_group('planning.group_planning_manager'):
                    record.mostrar_conformidad = True
                else:
                    record.mostrar_conformidad = False
    ...
    @api.onchange('cumplimiento_usuario')
    def _onchange_cumplimiento_usuario(self):
        if self.cumplimiento_usuario:
    ...
    @api.onchange('aceptar_prorroga')
    def _onchange_aceptar_prorroga(self):
        if self.aceptar_prorroga:
            self.write({
                'planning_prorrogation_ids': [(0, 0, {
                    'name': self.causa_prorroga,
                    'fecha_solicitud': self.fecha_solicitada,
                    'fecha_propuesta': self.nueva_fecha_propuesta,
                    # otros campos del modelo hijo
                })]
            })
            self.prorrogar = False
...
class PlanningSubComisiones(models.Model):
    _name = 'planning.subcomisiones'
...
class PlanningProrrogation(models.Model):
    _name = 'planning.prorrogation'

    name = fields.Text(string='Causa')
    fecha_solicitud = fields.Date(string='Fecha Solicitud')
    fecha_propuesta = fields.Date(string='Fecha Aprobada')
    planning_slot_id = fields.Many2one('planning.slot')


    # @api.model
    # def create(self, vals_list):
    #     # if vals_list['plan_trabajo']:
    #     #     # Buscar partner
    #     #     partner = self.env['resource.resource'].search([('id', '=', vals_list['resource_id'])])
    #     #     # Crear la tarea del Recurso en el Plan de Trabajo.
    #     #     self.env['calendar.event'].create({'name': 'Atención de Acuerdo No. ' + vals_list['nro_acuerdo'],
    #     #                                        'short_name': 'Acuerdo No. ' + vals_list['nro_acuerdo'],
    #     #                                        'start': vals_list['start_datetime'],
    #     #                                        'stop': fields.Datetime().to_string(fields.Datetime().from_string(vals_list['start_datetime']) + relativedelta(hours=1)),
    #     #                                        'partner_ids': partner.employee_id.user_id.partner_id,
    #     #                                        'description': vals_list['name']})
    #     #     if vals_list['ejecutor_id']:
    #     #         # Buscar partner
    #     #         partner = self.env['hr.employee'].search([('id', '=', vals_list['ejecutor_id'])])
    #     #         # Crear la tarea del Recurso en el Plan de Trabajo.
    #     #         self.env['calendar.event'].create({'name': 'Ejecución de Acuerdo No. ' + vals_list['nro_acuerdo'],
    #     #                                            'short_name': 'Ejecutar Acuerdo No. ' + vals_list['nro_acuerdo'],
    #     #                                            'start': vals_list['start_datetime'],
    #     #                                            'stop': fields.Datetime().to_string(fields.Datetime().from_string(vals_list['start_datetime']) + relativedelta(hours=1)),
    #     #                                            'partner_ids': partner.user_id.partner_id,
    #     #                                            'description': vals_list['name']})
        
    #     return super(Planning, self).create(vals_list)
