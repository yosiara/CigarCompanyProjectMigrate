...
class ITReports(models.Model):
    _name = 'it.reports'
    _description = 'IT Reports and Statistics'
    _auto = False  # Don't create DB table

    @api.model
    def get_dashboard_stats(self):
        """Get statistics for the dashboard"""
        Employee = self.env['it.employee']
        Computer = self.env['it.computer']
        Phone = self.env['it.phone']
        Service = self.env['it.service']
