...
import { createCommunityWebClient } from "@web_community/../tests/helpers";
import { homeMenuService } from "@web_community/webclient/home_menu/home_menu_service";
import { ormService } from "@web/core/orm_service";
import { browser } from "@web/core/browser/browser";
import { errorService } from "@web/core/errors/error_service";
import { session } from "@web/session";
import { shareUrlMenuItem } from "@web_community/webclient/share_url/share_url";
...
// Should test ONLY the webClient and features present in Community
// Those tests rely on hidden view to be in CSS: display: none
QUnit.module("WebClient Community", (hooks) => {
    hooks.beforeEach(() => {
    ...
    QUnit.module("basic flow with home menu", (hooks) => {
        ...
        QUnit.test("1 -- start up", async function (assert) {
            await createCommunityWebClient({ fixture, serverData, mockRPC });
        ...
        QUnit.test("restore the newly created record in form view", async (assert) => {
            ...
            const webClient = await createCommunityWebClient({ fixture, serverData });
    ...
    QUnit.test("clear unCommittedChanges when toggling home menu", async function (assert) {
        ...
        const webClient = await createCommunityWebClient({ fixture, serverData, mockRPC });
    ...
    QUnit.test("supports attachments of apps deleted", async function (assert) {
        ...
        await createCommunityWebClient({ fixture, serverData });
        assert.containsOnce(fixture, ".o_home_menu");
    });
