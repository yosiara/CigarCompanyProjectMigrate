...
class OrdersNoDeliveredReport(ReportXlsx):
    @api.model
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        work_order = self.env['maintenance_turei.work_order'].search(
            [('opening_date', '>=', lines.date_start), ('opening_date', '<=', lines.date_end), ('delivered', '=', False)], order='opening_date')

        worksheet.merge_range('A1:G1', tools.ustr("Resumen de Órdenes No Entregadas desde: %s hasta: %s") %(lines.date_start, lines.date_end), head_format)
...
OrdersNoDeliveredReport('report.maintenance_turei.orders_no_delivered_report', 'wzd.orders.no.delivered')
