...
class ReportController(main.ReportController):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        action_docxtpl_report = ir_action.get_from_report_name(
            reportname, "docxtpl").with_context(context)
        if not action_docxtpl_report:
            raise exceptions.HTTPException(
                description='Docx action report not found for report_name '
                            '%s' % reportname)
        context['report_name'] = reportname
        docxtpl_report = request.env['docxtpl.report'].create({
            'ir_actions_report_xml_id': action_docxtpl_report.id
        }).with_context(context)
        res, filetype = docxtpl_report.create_report(docids, data)
        filename = action_docxtpl_report.gen_report_download_filename(
            docids, data)
    ...
    def report_download(self, data, token):
        ...
        the download of a docxtpl/controller report.
        ...
        try:
            reportname = url.split('/report/docxtpl/')[1].split('?')[0]
            docids = None
            if '/' in reportname:
            ...
            if docids:
                # Generic report:
                response = self.report_routes(reportname, docids=docids, converter='docxtpl')
            else:
                # Particular report:
                # decoding the args represented in JSON
                data = url_decode(url.split('?')[1]).items()
                response = self.report_routes(
                    reportname, converter='docxtpl', **dict(data))
