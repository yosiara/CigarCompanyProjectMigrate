...
class AnnualServicePlan(models.Model):
    _name = 'atm.production_plan'
    _description = 'atm.production_plan'
    _rec_name = 'year'
    ...
    record_ids = fields.One2many('atm.plan_record', inverse_name='plan_id', string='Planes')


class AnnualServicePlanRecord(models.Model):
    _name = 'atm.plan_record'
    _description = 'atm.plan_record'

    plan_id = fields.Many2one('atm.production_plan', string='Plan')
