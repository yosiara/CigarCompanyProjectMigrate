...
try:
    ...
    logger.debug('Can not import docxtpl.')
...
def docxtpl_report_extender(report_xml_id=None):
    """
    ...
    If no report_xml_id is given the extender is registered for all docxtpl
    reports
...
class DocxTplReport(models.TransientModel):
    _name = "docxtpl.report"
    _inherit = 'report'
    _description = "Report Docx"
    ...
    def _is_valid_template_path(self, path):
        ...
        root_path = tools.config.get_misc('l10n_cu_report_docxtpl', 'root_tmpl_path')
        if not root_path:
            logger.warning(
        ...
        if not is_valid:
            logger.warning(
                "Docx template path is not valid. %s is not a child of root "
    ...
    def _is_valid_template_filename(self, filename):
        ...
        if filename and os.path.isfile(filename):
            ...
            ext = ext.replace('.', '')
            if ext != 'docx':
                logger.warning("Invalid docx template %s", filename, exc_info=1)
            else:
                return True
        logger.warning(
    ...
    def _get_template_from_path(self, tmpl_name):
        ...
        if self._is_valid_template_filename(flbk_filename):
            with open(flbk_filename, 'r+b') as tmpl:
                return tmpl.read()
        return None
    ...
    def get_template(self, model_instance):
        ...
        self.ensure_one()
        report_xml = self.ir_actions_report_xml_id
        if report_xml.docxtpl_template_id and report_xml.docxtpl_template_id.id:
            # if a user gave a report template
            tmpl_data = b64decode(
                report_xml.docxtpl_template_id.docxtpl_template_data
            )
    ...
    def _create_single_report(self, model_instance, data, save_in_attachment):
        ...
        """
        self.ensure_one()
        result_fd, result_path = tempfile.mkstemp(suffix='.docx', prefix='docxtpl.report.tmp.')
        ...
        with closing(os.fdopen(result_fd, 'w+')) as out_stream:
            ...
            tpl = DocxTemplate(in_stream)
            tpl.render(localcontext)
            if 'replace_media' in data:
                self._replace_media(tpl, data['replace_media'], localcontext)
            if 'replace_logo' in data:
                self._replace_logo(tpl, data['replace_logo'], localcontext)
            tpl.save(out_stream)
            out_stream.seek(0)
    ...
    def _replace_media(self, tpl, media, localcontext):
        for m in media:
            dst = False
            src = False
            if m['format_src'] == 'base64':
                src = StringIO(base64.decodestring(m['data_src']))
            elif m['format_src'] == 'path':
                src = m['data_src']
            if m['format_dst'] == 'base64':
                dst = StringIO(base64.decodestring(m['data_dst']))
            elif m['format_dst'] == 'path':
                dst = m['data_dst']
            if src and dst:
    ...
    def _replace_logo(self, tpl, data, localcontext):
        ...
        src = False
        if data['src'] == 'base64':
            src = StringIO(base64.decodestring(data['data']))
        elif data['src'] == 'path':
            src = data['data']
        dst = StringIO(base64.decodestring(localcontext['user'].company_id.logo))
        if src and dst:
            tpl.replace_media(src, dst)
    ...
    def _convert_single_report_cmd(self, result_path, model_instance, data):
        ...
        return [
            self.env['ir.config_parameter'].get_param(
                'docxtpl.conversion_command', 'libreoffice',
            ),
            '--headless',
            '--convert-to',
            self.ir_actions_report_xml_id.docxtpl_filetype,
            result_path,
        ]
    ...
    def _get_or_create_single_report(self, model_instance, data, save_in_attachment):
        ...
        if save_in_attachment and save_in_attachment['loaded_documents'].get(model_instance.id):
            d = save_in_attachment['loaded_documents'].get(model_instance.id)
            report_file = tempfile.mktemp("." + self.ir_actions_report_xml_id.docxtpl_filetype)
    ...
    def _zip_results(self, reports_path):
        ...
        result_path = tempfile.mktemp(suffix="zip", prefix='docxtpl-zip-result')
    ...
    def _merge_results(self, reports_path):
        ...
        if filetype == 'pdf':
            return self._merge_pdf(reports_path), 'pdf'
        else:
    ...
    def create_report(self, res_ids, data):
        ...
        reports_path = []
        if len(res_ids) > 1 and self.ir_actions_report_xml_id.docxtpl_multi_in_one:
            reports_path.append(self._create_single_report(model_instances, data, save_in_attachment))
        else:
            for model_instance in model_instances:
                reports_path.append(self._get_or_create_single_report(model_instance, data, save_in_attachment))
