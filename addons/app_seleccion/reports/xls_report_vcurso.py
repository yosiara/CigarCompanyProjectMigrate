...
class XlsReportVCurso(ReportXlsx):
    @api.model
    def generate_xlsx_report(self, workbook, data,lines):
        ...
        applicant_ids = self.env['hr.applicant'].search([('curso_habilitacion','=',lines.curso_id.id),('estado','=','proceso')])

        worksheet.merge_range('C1:G1', tools.ustr("Candidatos a verificar para "+lines.curso_id.name), head_format)
        ...
        worksheet.write('H3', tools.ustr('Puesto de Trabajo solicitado'), merge_format1)
        worksheet.write('I3', tools.ustr('Carrera'), merge_format1)
        ...
        for c in applicant_ids:
            ...
            if c.job_id:
                worksheet.write(x, 7, tools.ustr(c.nombre_trabajo), normal_format)
            else:
                worksheet.write(x, 7, tools.ustr(c.nombre_curso), normal_format)
            if c.degree_id:
                worksheet.write(x, 8, tools.ustr(c.degree_id.name), normal_format)
            else:
                worksheet.write(x, 8, tools.ustr(''), normal_format)

            x += 1



XlsReportVCurso('report.app_seleccion.xls_report_vcurso', 'app_seleccion.xls_report_vcurso')
