...
class FlowByLinesToExcelReport(ReportXlsx):
    @api.model
    def generate_xlsx_report(self, workbook, data, lines):

        worksheet = workbook.add_worksheet(tools.ustr('Eficiencia, CDT y Flujo por Líneas'))
        ...
        worksheet.merge_range('A1:I1', tools.ustr("REPORTE DE EFICIENCIA, CDT Y FLUJO POR LÍNEAS  DESDE: %s - HASTA: %s") %(lines.date_start, lines.date_end), head_format)
        ...
        worksheet.write('I2', tools.ustr('Flujo'), merge_format)
        ...
        for tc in tecnolog_control:
            ...
            worksheet.write(x, 4, tools.ustr(tc.productive_line.name), normal_format)
            ...
            div = (tc.plan_time -(tpexo / 60)) * tc.productive_capacity
            div_g = tc.production_in_production_system / div if div !=0 else 0
            efficiency_operative = div_g * 100
            ...
            worksheet.write(x, 8, round(tc.production_in_production_system / tc.execution_time,2) if tc.execution_time !=0 else 0, normal_format)
            x += 1

FlowByLinesToExcelReport('report.process_control_primary.flow_by_lines_report', 'wzd.flow.by.lines.to.excel')
